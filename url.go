...
func ParseURLs(text string) []string {
	...
	submatches := reURL.FindAllStringSubmatch(text, -1)
	matches := []string{}
	for _, match := range submatches {
		link := match[0]
		if match[1] == "" {
			// if no http - add http, check domain
			link = "http://" + link
			domainFull := strings.Split(match[2], ".")
			topDomain := domainFull[len(domainFull)-1]
			_, ok := urlAllowedDomains[strings.ToUpper(topDomain)]
			if !ok {
				continue
			}
...
var urlAllowedDomains = map[string]bool{
	"AAA":                      true,
	"AARP":                     true,
	"ABARTH":                   true,
	"ABB":                      true,
